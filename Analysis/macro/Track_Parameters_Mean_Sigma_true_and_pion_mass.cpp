...
void Track_Parameters_Mean_Sigma_true_and_pion_mass()
{

	gStyle->SetOptFit(1);
	ildStyle->SetOptStat(001111111);
	float leg_TextSize = 0.04;
	float leg_X1 = 0.57; float leg_Y1 = 0.70; float leg_X2 = 0.90; float leg_Y2 = 0.90;
...
//	LEGEND settings for all particles with p > 1 GeV
/*
	float d0_Mean_min = -0.1; float d0_Mean_max = 0.1; float d0_Sigma_min = 0.9; float d0_Sigma_max = 1.3;
	float Phi_Mean_min = -0.15; float Phi_Mean_max = 0.15; float Phi_Sigma_min = 0.9; float Phi_Sigma_max = 1.3;
	float Omega_Mean_min = -0.10; float Omega_Mean_max = 0.3; float Omega_Sigma_min = 1.0; float Omega_Sigma_max = 1.5;
	float tanLambda_Mean_min = -0.1; float tanLambda_Mean_max = 0.1; float tanLambda_Sigma_min = 0.8; float tanLambda_Sigma_max = 1.3;
	float z0_Mean_min = -0.1; float z0_Mean_max = 0.1; float z0_Sigma_min = 0.9; float z0_Sigma_max = 1.3;
	vector<int> mom = {3,5,10,15,25,50,100};
*/

	string file_dir = "/nfs/dust/ilc/user/radkhory/calib_samples/TrackerSamples/ParticleGun/FitFiles/";
	string plot_dir = "/afs/desy.de/group/flc/pool/radkhory/TrackParameters/Analysis/macro/Plots/Pulls_vs_angle_mom/";
	string input_tree_name = "TrackParameters";

	vector<int> angle = {15,25,35,45,55,65,75,85};
//	vector<int> angle = {15,25,35};
	vector<int> true_pdg = {11,13,211,321,2212};
//	vector<int> true_pdg = {11,13};
	...
	for (int mom_iter = 0; mom_iter < mom.size(); mom_iter++)
	{
		...
		TGraphErrors *graph_d0_Mean_true_mass[true_pdg.size()];
		TGraphErrors *graph_d0_Sigma_true_mass[true_pdg.size()];
		TGraphErrors *graph_d0_Mean_pion_mass[true_pdg.size()];
		TGraphErrors *graph_d0_Sigma_pion_mass[true_pdg.size()];
		...
		TGraphErrors *graph_Phi_Mean_true_mass[true_pdg.size()];
		TGraphErrors *graph_Phi_Sigma_true_mass[true_pdg.size()];
		TGraphErrors *graph_Phi_Mean_pion_mass[true_pdg.size()];
		TGraphErrors *graph_Phi_Sigma_pion_mass[true_pdg.size()];
		...
		TGraphErrors *graph_Omega_Mean_true_mass[true_pdg.size()];
		TGraphErrors *graph_Omega_Sigma_true_mass[true_pdg.size()];
		TGraphErrors *graph_Omega_Mean_pion_mass[true_pdg.size()];
		TGraphErrors *graph_Omega_Sigma_pion_mass[true_pdg.size()];
		...
		TGraphErrors *graph_tanLambda_Mean_true_mass[true_pdg.size()];
		TGraphErrors *graph_tanLambda_Sigma_true_mass[true_pdg.size()];
		TGraphErrors *graph_tanLambda_Mean_pion_mass[true_pdg.size()];
		TGraphErrors *graph_tanLambda_Sigma_pion_mass[true_pdg.size()];
		...
		TGraphErrors *graph_z0_Mean_true_mass[true_pdg.size()];
		TGraphErrors *graph_z0_Sigma_true_mass[true_pdg.size()];
		TGraphErrors *graph_z0_Mean_pion_mass[true_pdg.size()];
		TGraphErrors *graph_z0_Sigma_pion_mass[true_pdg.size()];
		for (int par_iter = 1; par_iter < true_pdg.size(); par_iter++)
		{
			string true_part = true_par[par_iter];
			char true_particle[true_part.size() + 1];
			strcpy(true_particle, true_part.c_str());

			string pion_part = pion_par[par_iter];
...
//							continue;
				...
				d0_true_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_d0_true_mass->GetFunction("gaus")->GetParameter(1); d0_true_mass_Mean_Err[angle_iter] = hist_pull_d0_true_mass->GetFunction("gaus")->GetParError(1);
				...
				d0_pion_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_d0_pion_mass->GetFunction("gaus")->GetParameter(1); d0_pion_mass_Mean_Err[angle_iter] = hist_pull_d0_pion_mass->GetFunction("gaus")->GetParError(1);
...
//				can_pull_d0->SaveAs((plot_dir+"Pulls/"+file_init[par_iter]+"_pull_d0_mom_"+to_string(mom[mom_iter])+"_Angle_"+to_string(angle[angle_iter])+".C").c_str());
				...
				Phi_true_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_Phi_true_mass->GetFunction("gaus")->GetParameter(1); Phi_true_mass_Mean_Err[angle_iter] = hist_pull_Phi_true_mass->GetFunction("gaus")->GetParError(1);
				...
				Phi_pion_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_Phi_pion_mass->GetFunction("gaus")->GetParameter(1); Phi_pion_mass_Mean_Err[angle_iter] = hist_pull_Phi_pion_mass->GetFunction("gaus")->GetParError(1);
...
//				can_pull_Omega->SaveAs((plot_dir+"Pulls/"+file_init[par_iter]+"_pull_Omega_mom_"+to_string(mom[mom_iter])+"_Angle_"+to_string(angle[angle_iter])+".C").c_str());
				...
				tanLambda_true_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_tanLambda_true_mass->GetFunction("gaus")->GetParameter(1); tanLambda_true_mass_Mean_Err[angle_iter] = hist_pull_tanLambda_true_mass->GetFunction("gaus")->GetParError(1);
				...
				tanLambda_pion_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_tanLambda_pion_mass->GetFunction("gaus")->GetParameter(1); tanLambda_pion_mass_Mean_Err[angle_iter] = hist_pull_tanLambda_pion_mass->GetFunction("gaus")->GetParError(1);
...
//				can_pull_tanLambda->SaveAs((plot_dir+"Pulls/"+file_init[par_iter]+"_pull_tanLambda_mom_"+to_string(mom[mom_iter])+"_Angle_"+to_string(angle[angle_iter])+".C").c_str());
				...
				z0_true_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_z0_true_mass->GetFunction("gaus")->GetParameter(1); z0_true_mass_Mean_Err[angle_iter] = hist_pull_z0_true_mass->GetFunction("gaus")->GetParError(1);
				...
				z0_pion_mass_Mean[angle_iter] = charge[par_iter] * hist_pull_z0_pion_mass->GetFunction("gaus")->GetParameter(1); z0_pion_mass_Mean_Err[angle_iter] = hist_pull_z0_pion_mass->GetFunction("gaus")->GetParError(1);
...
//				can_pull_z0->SaveAs((plot_dir+"Pulls/"+file_init[par_iter]+"_pull_z0_mom_"+to_string(mom[mom_iter])+"_Angle_"+to_string(angle[angle_iter])+".C").c_str());
			...
			graph_d0_Mean_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, d0_true_mass_Mean, 0, d0_true_mass_Mean_Err);
			graph_d0_Mean_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_d0_Mean_true_mass[par_iter]->SetLineStyle(1);
			graph_d0_Mean_true_mass[par_iter]->SetLineWidth(1);
			graph_d0_Mean_true_mass[par_iter]->SetMarkerSize(3);
			graph_d0_Mean_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_d0_Mean_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_d0_Mean->Add(graph_d0_Mean_true_mass[par_iter]);
			legend_d0_Mean->AddEntry(graph_d0_Mean_true_mass[par_iter], true_particle, "epl");

			graph_d0_Sigma_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, d0_true_mass_Sigma, 0, d0_true_mass_Sigma_Err);
			graph_d0_Sigma_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_d0_Sigma_true_mass[par_iter]->SetLineStyle(1);
			graph_d0_Sigma_true_mass[par_iter]->SetLineWidth(1);
			graph_d0_Sigma_true_mass[par_iter]->SetMarkerSize(3);
			graph_d0_Sigma_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_d0_Sigma_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_d0_Sigma->Add(graph_d0_Sigma_true_mass[par_iter]);
			legend_d0_Sigma->AddEntry(graph_d0_Sigma_true_mass[par_iter], true_particle, "epl");

			graph_Phi_Mean_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, Phi_true_mass_Mean, 0, Phi_true_mass_Mean_Err);
			graph_Phi_Mean_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Phi_Mean_true_mass[par_iter]->SetLineStyle(1);
			graph_Phi_Mean_true_mass[par_iter]->SetLineWidth(1);
			graph_Phi_Mean_true_mass[par_iter]->SetMarkerSize(3);
			graph_Phi_Mean_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Phi_Mean_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Phi_Mean->Add(graph_Phi_Mean_true_mass[par_iter]);
			legend_Phi_Mean->AddEntry(graph_Phi_Mean_true_mass[par_iter], true_particle, "epl");

			graph_Phi_Sigma_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, Phi_true_mass_Sigma, 0, Phi_true_mass_Sigma_Err);
			graph_Phi_Sigma_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Phi_Sigma_true_mass[par_iter]->SetLineStyle(1);
			graph_Phi_Sigma_true_mass[par_iter]->SetLineWidth(1);
			graph_Phi_Sigma_true_mass[par_iter]->SetMarkerSize(3);
			graph_Phi_Sigma_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Phi_Sigma_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Phi_Sigma->Add(graph_Phi_Sigma_true_mass[par_iter]);
			legend_Phi_Sigma->AddEntry(graph_Phi_Sigma_true_mass[par_iter], true_particle, "epl");

			graph_Omega_Mean_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, Omega_true_mass_Mean, 0, Omega_true_mass_Mean_Err);
			graph_Omega_Mean_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Omega_Mean_true_mass[par_iter]->SetLineStyle(1);
			graph_Omega_Mean_true_mass[par_iter]->SetLineWidth(1);
			graph_Omega_Mean_true_mass[par_iter]->SetMarkerSize(3);
			graph_Omega_Mean_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Omega_Mean_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Omega_Mean->Add(graph_Omega_Mean_true_mass[par_iter]);
			legend_Omega_Mean->AddEntry(graph_Omega_Mean_true_mass[par_iter], true_particle, "epl");

			graph_Omega_Sigma_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, Omega_true_mass_Sigma, 0, Omega_true_mass_Sigma_Err);
			graph_Omega_Sigma_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Omega_Sigma_true_mass[par_iter]->SetLineStyle(1);
			graph_Omega_Sigma_true_mass[par_iter]->SetLineWidth(1);
			graph_Omega_Sigma_true_mass[par_iter]->SetMarkerSize(3);
			graph_Omega_Sigma_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Omega_Sigma_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Omega_Sigma->Add(graph_Omega_Sigma_true_mass[par_iter]);
			legend_Omega_Sigma->AddEntry(graph_Omega_Sigma_true_mass[par_iter], true_particle, "epl");

			graph_tanLambda_Mean_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, tanLambda_true_mass_Mean, 0, tanLambda_true_mass_Mean_Err);
			graph_tanLambda_Mean_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_tanLambda_Mean_true_mass[par_iter]->SetLineStyle(1);
			graph_tanLambda_Mean_true_mass[par_iter]->SetLineWidth(1);
			graph_tanLambda_Mean_true_mass[par_iter]->SetMarkerSize(3);
			graph_tanLambda_Mean_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_tanLambda_Mean_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_tanLambda_Mean->Add(graph_tanLambda_Mean_true_mass[par_iter]);
			legend_tanLambda_Mean->AddEntry(graph_tanLambda_Mean_true_mass[par_iter], true_particle, "epl");

			graph_tanLambda_Sigma_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, tanLambda_true_mass_Sigma, 0, tanLambda_true_mass_Sigma_Err);
			graph_tanLambda_Sigma_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_tanLambda_Sigma_true_mass[par_iter]->SetLineStyle(1);
			graph_tanLambda_Sigma_true_mass[par_iter]->SetLineWidth(1);
			graph_tanLambda_Sigma_true_mass[par_iter]->SetMarkerSize(3);
			graph_tanLambda_Sigma_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_tanLambda_Sigma_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_tanLambda_Sigma->Add(graph_tanLambda_Sigma_true_mass[par_iter]);
			legend_tanLambda_Sigma->AddEntry(graph_tanLambda_Sigma_true_mass[par_iter], true_particle, "epl");

			graph_z0_Mean_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, z0_true_mass_Mean, 0, z0_true_mass_Mean_Err);
			graph_z0_Mean_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_z0_Mean_true_mass[par_iter]->SetLineStyle(1);
			graph_z0_Mean_true_mass[par_iter]->SetLineWidth(1);
			graph_z0_Mean_true_mass[par_iter]->SetMarkerSize(3);
			graph_z0_Mean_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_z0_Mean_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_z0_Mean->Add(graph_z0_Mean_true_mass[par_iter]);
			legend_z0_Mean->AddEntry(graph_z0_Mean_true_mass[par_iter], true_particle, "epl");

			graph_z0_Sigma_true_mass[par_iter] = new TGraphErrors(angle.size(), angles, z0_true_mass_Sigma, 0, z0_true_mass_Sigma_Err);
			graph_z0_Sigma_true_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_z0_Sigma_true_mass[par_iter]->SetLineStyle(1);
			graph_z0_Sigma_true_mass[par_iter]->SetLineWidth(1);
			graph_z0_Sigma_true_mass[par_iter]->SetMarkerSize(3);
			graph_z0_Sigma_true_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_z0_Sigma_true_mass[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_z0_Sigma->Add(graph_z0_Sigma_true_mass[par_iter]);
			legend_z0_Sigma->AddEntry(graph_z0_Sigma_true_mass[par_iter], true_particle, "epl");


			graph_d0_Mean_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, d0_pion_mass_Mean, 0, d0_pion_mass_Mean_Err);
			graph_d0_Mean_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_d0_Mean_pion_mass[par_iter]->SetLineStyle(7);
			graph_d0_Mean_pion_mass[par_iter]->SetLineWidth(1);
			graph_d0_Mean_pion_mass[par_iter]->SetMarkerSize(3);
			graph_d0_Mean_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_d0_Mean_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_d0_Mean->Add(graph_d0_Mean_pion_mass[par_iter]);
			legend_d0_Mean->AddEntry(graph_d0_Mean_pion_mass[par_iter], pion_particle, "epl");

			graph_d0_Sigma_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, d0_pion_mass_Sigma, 0, d0_pion_mass_Sigma_Err);
			graph_d0_Sigma_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_d0_Sigma_pion_mass[par_iter]->SetLineStyle(7);
			graph_d0_Sigma_pion_mass[par_iter]->SetLineWidth(1);
			graph_d0_Sigma_pion_mass[par_iter]->SetMarkerSize(3);
			graph_d0_Sigma_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_d0_Sigma_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_d0_Sigma->Add(graph_d0_Sigma_pion_mass[par_iter]);
			legend_d0_Sigma->AddEntry(graph_d0_Sigma_pion_mass[par_iter], pion_particle, "epl");

			graph_Phi_Mean_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, Phi_pion_mass_Mean, 0, Phi_pion_mass_Mean_Err);
			graph_Phi_Mean_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Phi_Mean_pion_mass[par_iter]->SetLineStyle(7);
			graph_Phi_Mean_pion_mass[par_iter]->SetLineWidth(1);
			graph_Phi_Mean_pion_mass[par_iter]->SetMarkerSize(3);
			graph_Phi_Mean_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Phi_Mean_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_Phi_Mean->Add(graph_Phi_Mean_pion_mass[par_iter]);
			legend_Phi_Mean->AddEntry(graph_Phi_Mean_pion_mass[par_iter], pion_particle, "epl");

			graph_Phi_Sigma_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, Phi_pion_mass_Sigma, 0, Phi_pion_mass_Sigma_Err);
			graph_Phi_Sigma_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Phi_Sigma_pion_mass[par_iter]->SetLineStyle(7);
			graph_Phi_Sigma_pion_mass[par_iter]->SetLineWidth(1);
			graph_Phi_Sigma_pion_mass[par_iter]->SetMarkerSize(3);
			graph_Phi_Sigma_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Phi_Sigma_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_Phi_Sigma->Add(graph_Phi_Sigma_pion_mass[par_iter]);
			legend_Phi_Sigma->AddEntry(graph_Phi_Sigma_pion_mass[par_iter], pion_particle, "epl");

			graph_Omega_Mean_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, Omega_pion_mass_Mean, 0, Omega_pion_mass_Mean_Err);
			graph_Omega_Mean_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Omega_Mean_pion_mass[par_iter]->SetLineStyle(7);
			graph_Omega_Mean_pion_mass[par_iter]->SetLineWidth(1);
			graph_Omega_Mean_pion_mass[par_iter]->SetMarkerSize(3);
			graph_Omega_Mean_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Omega_Mean_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_Omega_Mean->Add(graph_Omega_Mean_pion_mass[par_iter]);
			legend_Omega_Mean->AddEntry(graph_Omega_Mean_pion_mass[par_iter], pion_particle, "epl");

			graph_Omega_Sigma_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, Omega_pion_mass_Sigma, 0, Omega_pion_mass_Sigma_Err);
			graph_Omega_Sigma_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Omega_Sigma_pion_mass[par_iter]->SetLineStyle(7);
			graph_Omega_Sigma_pion_mass[par_iter]->SetLineWidth(1);
			graph_Omega_Sigma_pion_mass[par_iter]->SetMarkerSize(3);
			graph_Omega_Sigma_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Omega_Sigma_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_Omega_Sigma->Add(graph_Omega_Sigma_pion_mass[par_iter]);
			legend_Omega_Sigma->AddEntry(graph_Omega_Sigma_pion_mass[par_iter], pion_particle, "epl");

			graph_tanLambda_Mean_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, tanLambda_pion_mass_Mean, 0, tanLambda_pion_mass_Mean_Err);
			graph_tanLambda_Mean_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_tanLambda_Mean_pion_mass[par_iter]->SetLineStyle(7);
			graph_tanLambda_Mean_pion_mass[par_iter]->SetLineWidth(1);
			graph_tanLambda_Mean_pion_mass[par_iter]->SetMarkerSize(3);
			graph_tanLambda_Mean_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_tanLambda_Mean_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_tanLambda_Mean->Add(graph_tanLambda_Mean_pion_mass[par_iter]);
			legend_tanLambda_Mean->AddEntry(graph_tanLambda_Mean_pion_mass[par_iter], pion_particle, "epl");

			graph_tanLambda_Sigma_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, tanLambda_pion_mass_Sigma, 0, tanLambda_pion_mass_Sigma_Err);
			graph_tanLambda_Sigma_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_tanLambda_Sigma_pion_mass[par_iter]->SetLineStyle(7);
			graph_tanLambda_Sigma_pion_mass[par_iter]->SetLineWidth(1);
			graph_tanLambda_Sigma_pion_mass[par_iter]->SetMarkerSize(3);
			graph_tanLambda_Sigma_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_tanLambda_Sigma_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_tanLambda_Sigma->Add(graph_tanLambda_Sigma_pion_mass[par_iter]);
			legend_tanLambda_Sigma->AddEntry(graph_tanLambda_Sigma_pion_mass[par_iter], pion_particle, "epl");

			graph_z0_Mean_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, z0_pion_mass_Mean, 0, z0_pion_mass_Mean_Err);
			graph_z0_Mean_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_z0_Mean_pion_mass[par_iter]->SetLineStyle(7);
			graph_z0_Mean_pion_mass[par_iter]->SetLineWidth(1);
			graph_z0_Mean_pion_mass[par_iter]->SetMarkerSize(3);
			graph_z0_Mean_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_z0_Mean_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_z0_Mean->Add(graph_z0_Mean_pion_mass[par_iter]);
			legend_z0_Mean->AddEntry(graph_z0_Mean_pion_mass[par_iter], pion_particle, "epl");

			graph_z0_Sigma_pion_mass[par_iter] = new TGraphErrors(angle.size(), angles, z0_pion_mass_Sigma, 0, z0_pion_mass_Sigma_Err);
			graph_z0_Sigma_pion_mass[par_iter]->SetLineColor(color_code[par_iter]);
			graph_z0_Sigma_pion_mass[par_iter]->SetLineStyle(7);
			graph_z0_Sigma_pion_mass[par_iter]->SetLineWidth(1);
			graph_z0_Sigma_pion_mass[par_iter]->SetMarkerSize(3);
			graph_z0_Sigma_pion_mass[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_z0_Sigma_pion_mass[par_iter]->SetMarkerStyle(marker_code_pion[par_iter]);
			multi_z0_Sigma->Add(graph_z0_Sigma_pion_mass[par_iter]);
			legend_z0_Sigma->AddEntry(graph_z0_Sigma_pion_mass[par_iter], pion_particle, "epl");
		...
		can_d0_Mean->SaveAs((plot_dir+"Mean_Sigma/d0_Mean_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_d0_Sigma->SaveAs((plot_dir+"Mean_Sigma/d0_Sigma_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_Phi_Mean->SaveAs((plot_dir+"Mean_Sigma/Phi_Mean_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_Phi_Sigma->SaveAs((plot_dir+"Mean_Sigma/Phi_Sigma_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_Omega_Mean->SaveAs((plot_dir+"Mean_Sigma/Omega_Mean_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_Omega_Sigma->SaveAs((plot_dir+"Mean_Sigma/Omega_Sigma_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_tanLambda_Mean->SaveAs((plot_dir+"Mean_Sigma/tanLambda_Mean_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_tanLambda_Sigma->SaveAs((plot_dir+"Mean_Sigma/tanLambda_Sigma_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_z0_Mean->SaveAs((plot_dir+"Mean_Sigma/z0_Mean_mom_"+to_string(mom[mom_iter])+".pdf").c_str());
		...
		can_z0_Sigma->SaveAs((plot_dir+"Mean_Sigma/z0_Sigma_mom_"+to_string(mom[mom_iter])+".pdf").c_str());

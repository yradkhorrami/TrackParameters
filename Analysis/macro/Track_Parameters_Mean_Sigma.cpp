...
void Track_Parameters_Mean_Sigma()
{
...
//	LEGEND settings for all particles with p = 1 GeV
	...
	float d0_Mean_min = -0.2; float d0_Mean_max = 0.2; float d0_Sigma_min = 0.8; float d0_Sigma_max = 1.4;
	float Phi_Mean_min = -0.1; float Phi_Mean_max = 0.3; float Phi_Sigma_min = 0.8; float Phi_Sigma_max = 1.4;
	float Omega_Mean_min = -0.3; float Omega_Mean_max = 1.0; float Omega_Sigma_min = 1.0; float Omega_Sigma_max = 1.7;
	float tanLambda_Mean_min = -0.2; float tanLambda_Mean_max = 0.2; float tanLambda_Sigma_min = 0.8; float tanLambda_Sigma_max = 1.4;
	float z0_Mean_min = -0.2; float z0_Mean_max = 0.3; float z0_Sigma_min = 0.9; float z0_Sigma_max = 1.45;
	vector<int> mom = {1};

//	LEGEND settings for all particles with p > 1 GeV
/*
	float d0_Mean_min = -0.1; float d0_Mean_max = 0.1; float d0_Sigma_min = 0.9; float d0_Sigma_max = 1.3;
	float Phi_Mean_min = -0.1; float Phi_Mean_max = 0.1; float Phi_Sigma_min = 0.9; float Phi_Sigma_max = 1.3;
	float Omega_Mean_min = -0.15; float Omega_Mean_max = 0.25; float Omega_Sigma_min = 1.0; float Omega_Sigma_max = 1.5;
...
//	vector<int> true_pdg = {11,13};
	...
	vector<int> marker_code = {4,21,20,22,23};
	vector<string> true_par = {"e^{-}","#mu^{-}","#pi^{+}","K^{+}","p^{+}"};
	...
	for (int mom_iter = 0; mom_iter < mom.size(); mom_iter++)
	{
		...
		TGraphErrors *graph_d0_Mean[true_pdg.size()];
		TGraphErrors *graph_d0_Sigma[true_pdg.size()];
		...
		TGraphErrors *graph_Phi_Mean[true_pdg.size()];
		TGraphErrors *graph_Phi_Sigma[true_pdg.size()];
		...
		TGraphErrors *graph_Omega_Mean[true_pdg.size()];
		TGraphErrors *graph_Omega_Sigma[true_pdg.size()];
		...
		TGraphErrors *graph_tanLambda_Mean[true_pdg.size()];
		TGraphErrors *graph_tanLambda_Sigma[true_pdg.size()];
		...
		TGraphErrors *graph_z0_Mean[true_pdg.size()];
		TGraphErrors *graph_z0_Sigma[true_pdg.size()];
...
//				can_pull_z0->SaveAs((plot_dir+"Pulls/"+file_init[par_iter]+"_pull_z0_mom_"+to_string(mom[mom_iter])+"_Angle_"+to_string(angle[angle_iter])+".C").c_str());
			...
			graph_d0_Mean[par_iter] = new TGraphErrors(angle.size(), angles, d0_true_mass_Mean, 0, d0_true_mass_Mean_Err);
			graph_d0_Mean[par_iter]->SetLineColor(color_code[par_iter]);
			graph_d0_Mean[par_iter]->SetLineStyle(1);
			graph_d0_Mean[par_iter]->SetLineWidth(1);
			graph_d0_Mean[par_iter]->SetMarkerSize(3);
			graph_d0_Mean[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_d0_Mean[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_d0_Mean->Add(graph_d0_Mean[par_iter]);
			legend_d0_Mean->AddEntry(graph_d0_Mean[par_iter], true_particle, "epl");

			graph_d0_Sigma[par_iter] = new TGraphErrors(angle.size(), angles, d0_true_mass_Sigma, 0, d0_true_mass_Sigma_Err);
			graph_d0_Sigma[par_iter]->SetLineColor(color_code[par_iter]);
			graph_d0_Sigma[par_iter]->SetLineStyle(1);
			graph_d0_Sigma[par_iter]->SetLineWidth(1);
			graph_d0_Sigma[par_iter]->SetMarkerSize(3);
			graph_d0_Sigma[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_d0_Sigma[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_d0_Sigma->Add(graph_d0_Sigma[par_iter]);
			legend_d0_Sigma->AddEntry(graph_d0_Sigma[par_iter], true_particle, "epl");

			graph_Phi_Mean[par_iter] = new TGraphErrors(angle.size(), angles, Phi_true_mass_Mean, 0, Phi_true_mass_Mean_Err);
			graph_Phi_Mean[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Phi_Mean[par_iter]->SetLineStyle(1);
			graph_Phi_Mean[par_iter]->SetLineWidth(1);
			graph_Phi_Mean[par_iter]->SetMarkerSize(3);
			graph_Phi_Mean[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Phi_Mean[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Phi_Mean->Add(graph_Phi_Mean[par_iter]);
			legend_Phi_Mean->AddEntry(graph_Phi_Mean[par_iter], true_particle, "epl");

			graph_Phi_Sigma[par_iter] = new TGraphErrors(angle.size(), angles, Phi_true_mass_Sigma, 0, Phi_true_mass_Sigma_Err);
			graph_Phi_Sigma[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Phi_Sigma[par_iter]->SetLineStyle(1);
			graph_Phi_Sigma[par_iter]->SetLineWidth(1);
			graph_Phi_Sigma[par_iter]->SetMarkerSize(3);
			graph_Phi_Sigma[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Phi_Sigma[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Phi_Sigma->Add(graph_Phi_Sigma[par_iter]);
			legend_Phi_Sigma->AddEntry(graph_Phi_Sigma[par_iter], true_particle, "epl");

			graph_Omega_Mean[par_iter] = new TGraphErrors(angle.size(), angles, Omega_true_mass_Mean, 0, Omega_true_mass_Mean_Err);
			graph_Omega_Mean[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Omega_Mean[par_iter]->SetLineStyle(1);
			graph_Omega_Mean[par_iter]->SetLineWidth(1);
			graph_Omega_Mean[par_iter]->SetMarkerSize(3);
			graph_Omega_Mean[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Omega_Mean[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Omega_Mean->Add(graph_Omega_Mean[par_iter]);
			legend_Omega_Mean->AddEntry(graph_Omega_Mean[par_iter], true_particle, "epl");

			graph_Omega_Sigma[par_iter] = new TGraphErrors(angle.size(), angles, Omega_true_mass_Sigma, 0, Omega_true_mass_Sigma_Err);
			graph_Omega_Sigma[par_iter]->SetLineColor(color_code[par_iter]);
			graph_Omega_Sigma[par_iter]->SetLineStyle(1);
			graph_Omega_Sigma[par_iter]->SetLineWidth(1);
			graph_Omega_Sigma[par_iter]->SetMarkerSize(3);
			graph_Omega_Sigma[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_Omega_Sigma[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_Omega_Sigma->Add(graph_Omega_Sigma[par_iter]);
			legend_Omega_Sigma->AddEntry(graph_Omega_Sigma[par_iter], true_particle, "epl");

			graph_tanLambda_Mean[par_iter] = new TGraphErrors(angle.size(), angles, tanLambda_true_mass_Mean, 0, tanLambda_true_mass_Mean_Err);
			graph_tanLambda_Mean[par_iter]->SetLineColor(color_code[par_iter]);
			graph_tanLambda_Mean[par_iter]->SetLineStyle(1);
			graph_tanLambda_Mean[par_iter]->SetLineWidth(1);
			graph_tanLambda_Mean[par_iter]->SetMarkerSize(3);
			graph_tanLambda_Mean[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_tanLambda_Mean[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_tanLambda_Mean->Add(graph_tanLambda_Mean[par_iter]);
			legend_tanLambda_Mean->AddEntry(graph_tanLambda_Mean[par_iter], true_particle, "epl");

			graph_tanLambda_Sigma[par_iter] = new TGraphErrors(angle.size(), angles, tanLambda_true_mass_Sigma, 0, tanLambda_true_mass_Sigma_Err);
			graph_tanLambda_Sigma[par_iter]->SetLineColor(color_code[par_iter]);
			graph_tanLambda_Sigma[par_iter]->SetLineStyle(1);
			graph_tanLambda_Sigma[par_iter]->SetLineWidth(1);
			graph_tanLambda_Sigma[par_iter]->SetMarkerSize(3);
			graph_tanLambda_Sigma[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_tanLambda_Sigma[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_tanLambda_Sigma->Add(graph_tanLambda_Sigma[par_iter]);
			legend_tanLambda_Sigma->AddEntry(graph_tanLambda_Sigma[par_iter], true_particle, "epl");

			graph_z0_Mean[par_iter] = new TGraphErrors(angle.size(), angles, z0_true_mass_Mean, 0, z0_true_mass_Mean_Err);
			graph_z0_Mean[par_iter]->SetLineColor(color_code[par_iter]);
			graph_z0_Mean[par_iter]->SetLineStyle(1);
			graph_z0_Mean[par_iter]->SetLineWidth(1);
			graph_z0_Mean[par_iter]->SetMarkerSize(3);
			graph_z0_Mean[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_z0_Mean[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_z0_Mean->Add(graph_z0_Mean[par_iter]);
			legend_z0_Mean->AddEntry(graph_z0_Mean[par_iter], true_particle, "epl");

			graph_z0_Sigma[par_iter] = new TGraphErrors(angle.size(), angles, z0_true_mass_Sigma, 0, z0_true_mass_Sigma_Err);
			graph_z0_Sigma[par_iter]->SetLineColor(color_code[par_iter]);
			graph_z0_Sigma[par_iter]->SetLineStyle(1);
			graph_z0_Sigma[par_iter]->SetLineWidth(1);
			graph_z0_Sigma[par_iter]->SetMarkerSize(3);
			graph_z0_Sigma[par_iter]->SetMarkerColor(color_code[par_iter]);
			graph_z0_Sigma[par_iter]->SetMarkerStyle(marker_code[par_iter]);
			multi_z0_Sigma->Add(graph_z0_Sigma[par_iter]);
			legend_z0_Sigma->AddEntry(graph_z0_Sigma[par_iter], true_particle, "epl");
